...
| 8 | 8 月时令 6(扁豆 3 道 + 香菇炒茭白 / 红烧冬瓜 / 秋葵炒虾仁) | 已入库 |
| 9 | 番茄鸡蛋疙瘩汤(完整晚餐)+ 食材库补齐华东全年品类 24 项 | 已入库 |
| 10 | 无新菜谱;食材库 67 项补 `pack_hint`(克重 → 菜场说法),新增「今晚吃什么」页与一轮 UX 整改 | 已入库 |

**菜谱 77 道 / 食材 84 项,全部品类达标。**
...
食材库已跨季:夏季(秋葵、西葫芦、红薯叶、豇豆、毛豆、玉米、四季豆)、秋冬季(白菜、娃娃菜、茼蒿、菜心、莴笋、油麦菜、菠菜、白萝卜、西兰花、花菜、山药、芋头、茭白秋茭)、春季(荠菜、豌豆、春笋、枇杷)都有。
水果按全年补齐:苹果、梨、柑橘、猕猴桃、草莓、枇杷、火龙果、牛油果、无花果 + 原有夏季五种。
**当前 7 月在季可排的菜约占全库 2/3**,秋冬菜到季自动进池,无需改代码。

> 教训:第一版食材库是「7-8 月够用就行」建的,水果只有 6 项、秋冬蔬菜近乎空白,时令页一到淡季就是空的。**每月更新时不只是改 `season_months`,还要检查该季的品类是否成套**(叶菜/根茎/瓜茄/豆类/水果各有没有当季项)。

### 排菜评分(2026-08 已实现)

原来的「随机取候选 + 硬约束过滤」换成了打分取最高(权重见 `CLAUDE.md` 排菜约束一节)。无头跑 200 周:鱼/猪肝/面食/汤四条区间 **200/200 全部达标**(改之前靠 `enforceWeekly` 事后补位),当季专属菜占比约 78%,超 60 分钟且没有可提前菜的餐次约 3%(候选池确实排不开时才发生,硬约束优先)。

> **「每周一都是龙利鱼」是怎么来的(2026-08 修)**:两个原因叠加。① 排菜固定从周一顺序填,等于周一永远先挑,全库分最高的那道菜周周落在周一;② 营养缺口分当时是顶格 +5,周一 fish=0 直接触发,而周一不是采购日、`same_day` 的鲈鱼鲫鱼被硬约束挡掉,**能上的鱼只剩冷冻龙利鱼**。
> 修法:填充顺序每次随机打乱 + 缺口分改成二次曲线(周初几乎不加分)。300 周实测,鱼从「周一 100%」变成七天 19–42% 均匀分布,鲈鱼正确地只出现在周三/周日。
> **排查这类「总是同一道」的问题,先看填充顺序,再看有没有哪一项加分在早期就顶格。**
