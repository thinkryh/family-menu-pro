...
**定位:不替代周菜单,是「今天临时不想吃排好的那道」时的逃生口。**把「从 77 道里挑」压缩成「从 3 张里挑」——所以一次只发 3 张。

- 候选复用排菜那套硬约束:不隔夜的鲜货只在采购当天出现、本周已排过的不再抽、汤不作主菜抽、条件槽(辣度/耗时/荤素)作临时过滤
- 评分偏向当季 + 快手 + 宝宝可吃 + 用得上同批次已买的食材,取前段加权随机
- **三张必须类别不同**(荤/素/主食),且在各类别候选里随机取——只取分最高的会导致每次抽出来都是同几道
- 每天限 3 次重抽,存 localStorage。这是刻意的:无限重抽等于把全库又翻一遍,抽卡就失去决策价值
- 卡片可翻面,背面是这道菜的宝宝分菜时机;选中写入今天晚餐对应位置,菜单页可「撤销」
