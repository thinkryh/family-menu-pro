...
目标 70 道:阜阳家常 30(约 20 荤、10 素/小菜)+ 补充荤菜 10 + 时令素菜 20 + 宝宝重点菜 5 + 面食主食 5。
排除项:臭鳜鱼及重口徽州菜。辣菜正常入库(标 spicy 等级)。
...
- 单餐总耗时 ≤ 60 分钟
- 每周:猪肝 1 次(少量,宝宝补铁)、鱼 ≥ 2 次(优先宝宝可用鱼种)
  → 随机排菜不保证命中,由 `enforceWeekly()` 在生成后补位;补位优先于「同主料 ≤2 次」的软约束
- 鲜鱼排在采购日 ±1 天;叶菜按采购批次分配,不跨批
